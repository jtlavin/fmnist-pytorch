...
winner_hist = train_and_evaluate(winner_config, training_data, test_data, device)

# Plotting
plt.figure(figsize=(10, 6))
plt.plot(baseline_hist['accuracy'], label='Baseline (SGD, ReLU, 64 batch)')
plt.plot(winner_hist['accuracy'], label='Winner (Adam, Tanh, 32 batch)')
plt.xlabel('Epochs')
plt.ylabel('Validation Accuracy')
plt.title('Baseline vs optimized Model (10 Epochs)')
plt.legend()
plt.grid(True)
plt.savefig('comparison_baseline_vs_winner.png')
print("Saved comparison plot to comparison_baseline_vs_winner.png")
...
print(f"Moved {len(png_files)} plot files to {plots_dir}/")
